...
The images has been annotated using the website \url{https://dataloop.ai/} which return the annotations on a dataset of JSON files, one for each image, containing all the labels information. To parse all the necessary information contained in the JSON files a dedicated class in C++, which makes use of the library \code{json.hpp}\footnote{\url{https://github.com/nlohmann/json}}, named \code{labeltxt.cpp \& labeltxt.h}, has been created to convert the annotations provided in a txt files in the format \newline {\fontfamily{qcr}\selectfont
classNumber x y width height} where the {\fontfamily{qcr}\selectfont
classNumber} is nothing but the object class mapped to an integer, therefore for the current project specification it will be just $0$, while {\fontfamily{qcr}\selectfont
x y width height} are the coordinates of the box center, its width and its height, normalized from $0$ to $1$ with respect to the image size.\newline
